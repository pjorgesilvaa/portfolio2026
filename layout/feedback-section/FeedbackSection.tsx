...
export default async function FeedbackSection() {
  ...
  return (
    <div className="w-full md:w-7xl md:px-8">
      ...
      <div className="flex flex-col lg:flex-row justify-center items-center gap-8 lg:gap-16 mt-8 lg:mt-32">
        {t.feedback.items.map((item, index) => (
          <AnimateIn
            key={index}
            animation="fade-up"
            delay={index * 120}
            className={`w-full ${index === 1 ? 'lg:mt-32' : ''}`}
          >
            <div className="lg:min-h-110 bg-white rounded-lg shadow-md overflow-hidden p-8 flex flex-col justify-between items-start gap-4 hover:-translate-y-2 hover:shadow-xl transition-all duration-300">
